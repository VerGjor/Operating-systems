...
public class TemplateNumRunsAndNumInstances {
	...
	static Semaphore lock, clients, wakeUpBarber, finished, cHere, cCalled, cInside;
    static int count = 0;
    static boolean isSleeping = true;
    /**
    ...
    public static void init(int numBarbers) {
    	...
    	wakeUpBarber = new Semaphore(0);
    	finished = new Semaphore(0);
    	cHere = new Semaphore(0);
    	cCalled = new Semaphore(0);
    	cInside = new Semaphore(0);
    }
    ...
    static class Consumer extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
           ...
           lock.acquire();
           count++;
           if(count == 5 && isSleeping) {
        	   wakeUpBarber.release(5);
           }
           lock.release();
